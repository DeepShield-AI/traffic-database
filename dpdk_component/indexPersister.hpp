...
private:
    ...
    const u_int64_t key_lens[IndexType::TOTAL_INDEX] = {sizeof(u_int32_t), sizeof(u_int32_t), sizeof(u_int16_t), sizeof(u_int16_t), sizeof(IPv6Address), sizeof(IPv6Address)};
    ...
    std::vector<IndexMemoryPool*>* memoryPools;
...
public:
    IndexPersister(u_int64_t disk_size, u_int64_t block_size, IndexBuffer* indexBuffer, IndexBlockBuffer* indexBlockBuffer, DiskBuffer* diskBuffer,std::vector<IndexMemoryPool*>*memoryPools, std::atomic_uint64_t* diskWritePos, bool bind_core = false, u_int32_t core_id = 0):
